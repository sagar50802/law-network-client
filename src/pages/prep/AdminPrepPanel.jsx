...
export default function AdminPrepPanel() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <div className="rounded-xl border bg-white p-4 mb-6">
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <select className="w-full border rounded px-3 py-2 bg-white" value={selExam} onChange={e=>setSelExam(e.target.value)}>
          ...
          <form onSubmit={createExam} className="grid md:grid-cols-3 gap-2">
            <div>
              ...
              <input className="w-full border rounded px-3 py-2" placeholder="UP_APO"
                     value={makeExam.examId} onChange={e=>setMakeExam(v=>({ ...v, examId: e.target.value }))}/>
            </div>
            <div>
              <label className="block text-sm mb-1">Name</label>
              <input className="w-full border rounded px-3 py-2" placeholder="UP APO"
                     value={makeExam.name} onChange={e=>setMakeExam(v=>({ ...v, name: e.target.value }))}/>
            </div>
      ...
      {!selExam ? <div className="text-gray-500">Select or create an exam to continue.</div> : <ExamEditor examId={selExam} />}
    </div>
  );
}
...
function ExamEditor({ examId }) {
  ...
  return (
    <>
      ...
      <form ref={formRef} onSubmit={onSave} encType="multipart/form-data" className="rounded-xl border bg-white p-4 mb-6 grid gap-3">
        ...
        <div>
          ...
          <input name="releaseAt" type="datetime-local" className="border rounded px-3 py-2"/>
        </div>
        ...
        <div>
          ...
          <textarea name="manualText" rows={6} placeholder="Paste text here…" className="w-full border rounded px-3 py-2"/>
        </div>

        <div className="grid md:grid-cols-5 gap-3">
          <label className="flex items-center gap-2"><input type="checkbox" name="extractOCR"/> Extract OCR</label>
          <label className="flex items-center gap-2"><input type="checkbox" name="showOriginal"/> Show Original</label>
          <label className="flex items-center gap-2"><input type="checkbox" name="allowDownload"/> Allow Download</label>
          <label className="flex items-center gap-2"><input type="checkbox" name="highlight"/> Highlight</label>
          <input className="w-full border rounded px-3 py-2" name="background" placeholder="background (e.g. #fffbe7)"/>
        </div>

        <div className="flex gap-2">
          <button disabled={busy} className="px-4 py-2 rounded bg-black text-white">{busy ? "Saving…" : "Save Module"}</button>
            ...
            <div key={m._id} className="px-4 py-3 text-sm flex items-center justify-between">
              <div>
                ...
                <div className="text-gray-500">
                  ...
                  {m.releaseAt ? ` • ${m.status} @ ${new Date(m.releaseAt).toLocaleString()}` : ""}
                </div>
              </div>
              <button onClick={()=>onDelete(m._id)} className="text-red-600 border px-2 py-1 rounded">Delete</button>
            </div>
          ))}
