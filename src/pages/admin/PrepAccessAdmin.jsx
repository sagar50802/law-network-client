...
export default function PrepAccessAdmin() {
  ...
  // Auto-approval toggle (exam.autoGrantRestart)
  ...
  // Load per-exam auto-approve flag
  useEffect(() => {
    ...
    <div className="max-w-5xl mx-auto p-4">
      ...
      {err && (
        <div className="text-sm text-rose-700 bg-rose-50 border border-rose-200 rounded p-2 mb-3">
          {err}
          <div className="opacity-70 mt-1">Make sure <code>X-Owner-Key</code> is set as <code>VITE_OWNER_KEY</code>.</div>
        </div>
      )}
            ...
            return (
              <div key={x._id} className="border rounded p-3">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-medium">{String(x.intent || "").toUpperCase()} • {x.examId}</div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {status === "pending" && (
                    <>
                      <button className="px-3 py-1 rounded bg-emerald-600 text-white" onClick={()=>approve(x._id, true)}>
                        Approve & Grant
                      </button>
                      <button className="px-3 py-1 rounded bg-rose-600 text-white" onClick={()=>approve(x._id, false)}>
                        Reject
                      </button>
                    </>
                  )}
                  <button className="px-3 py-1 rounded border" onClick={()=>revokeRow(x)} title="Revoke the user’s existing access">
                    Revoke Access
                  </button>
                </div>
              </div>
