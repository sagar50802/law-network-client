...
async function uploadSlide({
  ...
  if (intro) fd.append("intro", intro);
...
export default function ConsultancySection({ autoScroll = true }) {
  ...
  async function onReplace(s, newFile) {
    ...
    setBusy(true);
    try {
      await patchSlide(s.id || s._id, { file: newFile });
      await load();
    } catch (err) {
      console.error(err);
  ...
  async function onDelete(s) {
    ...
    setBusy(true);
    try {
      await delSlide(s.id || s._id);
      setItems((p) =>
        p.filter((x) => (x.id || x._id) !== (s.id || s._id))
      );
    } catch (err) {
      console.error(err);
        ...
        <IfOwnerOnly>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() =>
                getJSON("/api/consultancy").then((r) =>
                  setItems(r.items || [])
                )
              }
              disabled={busy}
  ...
  return (
    <div className="w-full rounded-2xl border bg-white shadow-sm ring-1 ring-black/5">
      <div className="relative p-2">
        <div className="h-[260px] md:h-[280px] rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center overflow-hidden ring-1 ring-gray-200">
          {item.image ? (
            <SmartImg
              src={item.image}
          ...
          <div className="mt-3 flex items-center gap-3 flex-wrap">
            ...
            {links.website && (
              <IconButton
                label="Open Website"
                onClick={() =>
                  window.open(links.website, "_blank", "noopener")
                }
              >
                <LinkSVG />
              </IconButton>
...
function IconButton({ label, onClick, children }) {
  return (
    <button
      ...
      title={label}
      aria-label={label}
      className="inline-flex items-center justify-center w-9 h-9 rounded-full border hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-400 transition"
    >
      {children}
    </button>
  );
...
function TelegramSVG() {
  return (
    <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
      ...
      <path
        fill="#fff"
        d="M17.6 8.1L15.9 16c-.13.58-.49.72-.99.45l-2.7-1.99-1.3 1.25c-.14.14-.26.26-.53.26l.19-2.73 4.97-4.49c.22-.19-.05-.3-.34-.1l-6.16 3.88-2.64-.83c-.57-.18-.58-.58.12-.86l10.3-3.98c.47-.17.88.12.73.86z"
      />
    </svg>
  );
}
...
function LinkSVG() {
  return (
    <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
      <path
        d="M10 13a5 5 0 007.07 0l1.41-1.41a5 5 0 00-7.07-7.07L9 6"
        fill="none"
        stroke="#111827"
        strokeWidth="2"
      />
      <path
        d="M14 11a5 5 0 01-7.07 0L5.5 9.57a5 5 0 017.07-7.07L15 4"
        fill="none"
        stroke="#111827"
        strokeWidth="2"
      />
