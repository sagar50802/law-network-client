...
const readLS = (k, f) => {
  try { return JSON.parse(localStorage.getItem(LS_PREFIX + k)) ?? f; } catch { return f; }
};
const writeLS = (k, v) => { try { localStorage.setItem(LS_PREFIX + k, JSON.stringify(v)); } catch {} };
...
/* ─────────────── MAIN FORM MODAL ─────────────── */
const emptyProposal = {
  name: "", gender: "Other", nationality: "", place: "", university: "",
  stream: "", researchAreaDept: "", researchAreaSubject: "", title: "",
  abstract: "", pages: "", labOption: "",
};
...
function StudentForm({ onClose }) {
  ...
  const formComplete =
    data.name && data.nationality && data.place && data.university &&
    data.stream && data.researchAreaDept && data.researchAreaSubject &&
    data.title && data.abstract && data.pages;

  const save = () => writeLS("proposal:data", data);

  useEffect(save, [data]);

  const handleContinue = () => {
    save();
    if (data.labOption === "self") navigate("/research-nav/journey");
    else onClose?.();
  };
      ...
      >
        ...
        <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: THEME.border }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs" style={{ color: THEME.inkSoft }}>Step {step + 1}/2</div>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 pb-5 max-h-[70vh] overflow-auto mt-4">
          <AnimatePresence mode="wait">
            {step === 0 && (
              <motion.div key="form1" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                <div className="grid md:grid-cols-2 gap-4">
                  <Field label="Full Name" value={data.name} onChange={(v) => setData({ ...data, name: v })} />
                  <Select label="Gender" value={data.gender} onChange={(v) => setData({ ...data, gender: v })} options={["Male", "Female", "Other"]} />
                  ...
                  <Select label="Research Area (Govt Dept)" value={data.researchAreaDept} onChange={(v) => setData({ ...data, researchAreaDept: v })} options={GOV_DEPTS} />
                  <Select label="Research Area (UGC Subject)" value={data.researchAreaSubject} onChange={(v) => setData({ ...data, researchAreaSubject: v })} options={UGC_SUBJECTS} />
                  <div className="md:col-span-2">
                    <Field label="Research Title" value={data.title} onChange={(v) => setData({ ...data, title: v })} />
                  </div>
                  <div className="md:col-span-2">
                    <TextArea label="Abstract / Summary" rows={5} value={data.abstract} onChange={(v) => setData({ ...data, abstract: v })} />
                  </div>
                  <Field label="Required Page Count" type="number" value={data.pages} onChange={(v) => setData({ ...data, pages: v })} />
                </div>
              </motion.div>
            )}

            {step === 1 && (
              <motion.div key="form2" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
        ...
        {/* FOOTER */}
        <div className="px-5 py-4 border-t flex items-center justify-end gap-2" style={{ borderColor: THEME.border }}>
          {step === 0 ? (
            <button
              onClick={() => setStep(1)}
              disabled={!formComplete}
...
function LabOption({ title, desc, selected, onSelect, disabled }) {
  return (
    <button
      disabled={disabled}
      onClick={onSelect}
      className={`text-left rounded-xl p-4 border transition-all ${selected ? "ring-2" : ""} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
      style={{
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {selected && <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#033532", border: `1px solid ${THEME.border}`, color: THEME.aqua }}>Selected</span>}
      </div>
      <div className="text-sm mt-1" style={{ color: THEME.inkSoft }}>{desc}</div>
    </button>
  );
}
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        <div className="rounded-2xl border shadow-lg p-6" style={{ background: THEME.card, borderColor: THEME.border }}>
      ...
      <AnimatePresence>{showForm && <StudentForm onClose={() => setShowForm(false)} />}</AnimatePresence>
    </div>
  );
}
