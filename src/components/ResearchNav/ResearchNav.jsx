...
function gate(form) {
  ...
  // Payment only unlocks once Method is complete (keeps it strict)
  const mOK = (STEPS.find(x => x.id === "method").need || []).every(r => !!form[r]);
...
/* ─────────────────── Vertical “Water” Track (strict gating) ─────────────────── */
function WaterTrack({ gates, activeId, onClick }) {
  const visible = gates.filter(s => s.state !== "locked"); // only current + previous
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    active &&
                      "bg-white text-[#45b1ff] border-2 border-[#45b1ff] shadow-[0_0_26px_rgba(79,181,255,.45)]",
                    done &&
                      "bg-[#27cda2] text-white shadow-[0_0_0_12px_rgba(39,205,162,.18)]",
                    !active && !done && "bg-white text-[#45b1ff] border border-[#bfe3ff]",
                  ].join(" ")}
                  title={STEPS.find(x => x.id === s.id)?.label}
                >
                  <span className="animate-ripple text-[16px]">
          ...
          {visible.slice(-1).map((s) => {
            ...
            return (
              <div key={s.id} className="mb-[110px] last:mb-0">
                ...
                {/* Hint only (does not reveal next titles) */}
...
/* ─────────────── Route Summary (Google-Maps vibe, strict) ─────────────── */
      ...
      <div className="flex items-start gap-3 opacity-70">
        ...
        <div className="flex-1">
          ...
          <div className="text-sm" style={{ color: PALETTE.inkSoft }}>
            {nextSteps
              .slice(1)
              .map(s => STEPS.find(x => x.id === s.id)?.label)
              .filter(Boolean)
              .join(" → ") || "—"}
          </div>
        </div>
...
/* ───────────── Bottom Sticky Mini-Nav (Maps-style) ───────────── */
...
/* ───────────── Preview panel (sticky on xl; scrolls, no overlap) ───────────── */
function PreviewPanel({ children }) {
  const [open, setOpen] = useState(true);
  return (
    <div className="w-full xl:sticky xl:top-[92px] xl:h-[calc(100vh-120px)]">
      ...
      <div className={`${open ? "block" : "hidden"} xl:block xl:h-full xl:overflow-auto`}>
        {children}
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl shadow-sm p-6" style={{ background: PALETTE.card, border: `1px solid ${PALETTE.border}` }}>
      ...
      {id === "payment" && (
        <div className="grid gap-4">
          <div
            className="rounded-xl p-4 backdrop-blur-lg shadow"
            style={{ background: "rgba(255,255,255,.55)", border: `1px solid ${PALETTE.border}` }}
          >
            <div className="text-base font-semibold mb-2" style={{ color: PALETTE.inkSoft }}>
              Payment
            </div>
            <div className="grid sm:grid-cols-[120px_1fr] gap-4 items-center">
              {/* QR slot */}
              <div
                className="h-[120px] w-[120px] rounded-lg bg-white grid place-items-center"
                style={{ border: `1px solid ${PALETTE.border}`, boxShadow: "inset 0 0 0 1px rgba(255,255,255,.6)" }}
              >
                <div className="text-[10px]" style={{ color: PALETTE.inkSoft }}>
                  QR
                </div>
              </div>
              <div>
                ...
                >
                  <input
                    ...
                    className="hidden"
                    onChange={(e) =>
                      setForm(f => ({ ...f, paymentProofName: e.target.files?.[0]?.name || "" }))
                    }
                  />
            ...
            <div className="mt-3">
              <div className="h-2 w-full rounded-full bg-[#eee2c8] overflow-hidden">
                <div
                  ...
                  style={{ width: form.paymentVerified ? "100%" : "35%", background: PALETTE.blue }}
                />
              </div>
...
/* ─────────────── Stage Summary & End Ceremony (with close button) ─────────────── */
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm grid place-items-center p-4">
      ...
      >
        ...
        <div
          ...
          style={{ borderColor: PALETTE.border, background: "#fffdf7" }}
        >
          <h3 className="text-lg font-bold" style={{ color: PALETTE.ink }}>
            Stage Summary
          </h3>
          <button
            onClick={onClose}
            className="px-3 py-1.5 rounded-md text-sm"
            style={{ border: `1px solid ${PALETTE.border}`, background: "#ffffffb3", color: PALETTE.ink }}
          >
            Close
          </button>
        </div>
        ...
        <div className="px-6 pt-4 pb-6">
          ...
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            <div className="rounded-xl p-3" style={{ border: `1px solid ${PALETTE.border}`, background: "#ffffffb8" }}>
              ...
              <div className="font-medium break-words" style={{ color: PALETTE.ink }}>
                {form.title || "—"}
              </div>
            </div>
            <div className="rounded-xl p-3" style={{ border: `1px solid ${PALETTE.border}`, background: "#ffffffb8" }}>
            ...
            <div className="rounded-xl p-3" style={{ border: `1px solid ${PALETTE.border}`, background: "#ffffffb8" }}>
              ...
              <div className="font-mono whitespace-pre-wrap break-words" style={{ color: PALETTE.ink }}>
                {form.lit || "—"}
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-20 bg-white/65 backdrop-blur border-b" style={{ borderColor: PALETTE.border }}>
        <div className="max-w-6xl mx-auto p-4">
          <div className="flex items-center justify-between">
            <div className="text-lg font-extrabold" style={{ color: PALETTE.ink }}>
              Research Navigation
            </div>
      ...
      <main className="mx-auto w-full max-w-6xl px-4 sm:px-6 py-4 grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        {/* Right column: Preview (sticky) + Route Summary */}
