...
export default function PrepAccessOverlay({ examId, email }) {
  ...
  // NEW: hide only the panel content (keep blur/veil to gate interaction)
  const [panelHidden, setPanelHidden] = useState(false);

  /** ✅ Updated fetchStatus: keeps behavior, adds planDay+time auto-open */
  ...
  // ✅ OPTIONAL: Auto-recheck every 60s so it can open right at the minute (no manual refresh)
  useEffect(() => {
  ...
  function closeForToday() {
    ...
    // Hide only the panel, keep page blocked.
    setPanelHidden(true);
  }

  // If veil is off, render nothing (allow interaction)
  ...
  const desc =
    state.mode === "purchase"
      ...
      : state.mode === "restart"
      ? `You've completed all ${state.access?.planDays || ""} day(s). Restart "${
          state.exam?.name || examId
        }" from Day 1.`
