...
export default function PrepAccessOverlay({ examId, email }) {
  ...
  function buildPayMeta() {
    ...
    const waText = (
      pay.whatsappText ||
      `Hello, I paid for "${courseName}" (₹${priceINR}).`
    ).trim();
  ...
  async function submitRequest() {
    ...
    fd.append(
      "intent",
      state.mode === "restart" ? "restart" : "purchase"
    );
        ...
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-5">
          ...
          {state.mode === "approved" && (
            <>
              <div className="text-sm text-emerald-700 mb-3">
                Your access has been approved. Unlocking in ~
                {approveLeft || 15}s.
              </div>
              <button
              ...
              <div className="text-[11px] text-gray-500">
                After approval, your schedule restarts from Day 1.
              </div>
            </>
          )}
          ...
          {state.mode === "waiting" && (
            <>
              ...
              <div className="text-[11px] text-gray-500">
                This window will auto-unlock when approved.
              </div>
            </>
          )}
            ...
            <>
              ...
              {(upiLeft > 0 || waLeft > 0) && (
                <div className="text-[12px] text-gray-700 mb-3">
                  {upiLeft > 0 && (
                    <div className="mb-1">
                      After paying, <b>return to this tab</b> to finish.
                      Auto-focus in ~{upiLeft}s.
                    </div>
                  )}
                  {waLeft > 0 && (
                    <div>
                      After sending screenshot on WhatsApp,{" "}
                      <b>come back here</b>. We’ll bring you back in ~{waLeft}s.
                    </div>
                  )}
                </div>
              )}
